  ...
  try {
    ...
    if (!chapter.manga.value!.isLocalArchive!) {
      if ((isarPageUrls?.urls?.isNotEmpty ?? false) &&
        ...
        // Only ask the source when there is nothing on disk to read.
        final manga = chapter.manga.value!;
        final src = getSource(manga.lang!, manga.source!, manga.sourceId);
        // Built-in manga sources: bypass isolate service to avoid Isar deadlock.
        if (src != null && BuiltInSources.isBuiltIn(src)) {
          final bi = BuiltInSources.getForSource(src)!;
